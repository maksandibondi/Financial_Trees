...
%% Main program
[V,S,u,d] = price_s0_fixed_AM(N,T,S0,K,r,sigma);
Initial_price = V(1,1);
...
S01 = 0:1:20;
for i = 1:size(S01,2);
    [V1,S1,u1,d1] = price_s0_fixed_AM(N,T,S01(i),K,r,sigma);
    Initial_price1(i) = V1(1,1);
end;
figure; plot(S01,Initial_price1);
hold on; plot(S(N+1,:),V(N+1,:)); xlabel('variable S'); ylabel('variable V'); legend('initial price', 'final condition');
...
for i = 1:N+1
    for k = 1:N+1
        [V,S,u,d] = price_s0_fixed_AM(N,T-vector_t(i),S_new(k),K,r,sigma);
        V_new(i,k) = V(1,1);
        k = k+1;
    end;
i = i+1;
